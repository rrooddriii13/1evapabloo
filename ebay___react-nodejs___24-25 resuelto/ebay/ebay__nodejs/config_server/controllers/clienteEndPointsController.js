...
module.exports={
    ...
    ActualizarDirecciones: async function(req,res,next){
        try {
            ...
            direcciones=direcciones.map( direc => { const {operacion, pos, ...resto}=direc; return resto } );  

            let _resUpdate=await clienteMongoDB.db(process.env.DB_MONGODB)
                                                .collection('clientes')
                                                .findOneAndUpdate( { _id: new BSON.ObjectId(idCliente) }, { $set:{ 'cliente.direcciones': direcciones } }, { returnDocument: 'after' } );
            console.log('documento cliente actualizado a nuevas direcciones...', _resUpdate);

            res.status(200).send( { codigo: 0, mensaje : 'direcciones actualizadas oks...' + error.message } );

        } catch (error) {
            console.log('error al actualizar direcciones del cliente...', error);
            res.status(200).send( { codigo: 20, mensaje : 'error al actualizar direcciones del cliente...' + error.message } );
        }
    }
}